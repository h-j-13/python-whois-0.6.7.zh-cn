...
def cast_date(s, dayfirst=False, yearfirst=False):
    ...
    if DATEUTIL:
        try:
            return dp.parse(
                s, tzinfos=tz_data, dayfirst=dayfirst,
                yearfirst=yearfirst).replace(tzinfo=None)
        except Exception:
...
class WhoisEntry(dict):
    ...
    def __init__(self, domain, text, regex=None):
        if 'This TLD has no whois server, but you can access the whois database at' in text:
            ...
            self.domain = domain
            self.text = text
            if regex is not None:  #　找不到专用解析模板则返回通用解析模板
                self._regex = regex
            self.parse()

    def parse(self):  # 根据解析模板提取WHOIS数据
        """The first time an attribute is called it will be calculated here.
        The attribute is then set to be accessed directly by subsequent calls.
        """
        for attr, regex in list(self._regex.items()):  # 提取所有解析模板及对于的关键词
            if regex:
                values = []
                for data in re.findall(regex, self.text, re.IGNORECASE):
                    matches = data if isinstance(data, tuple) else [data]
                    for value in matches:
                        value = self._preprocess(attr, value)  # 处理匹配的字符串
                        if value and value not in values:
                            # avoid duplicates
                            values.append(value)
                if values and attr in ('registrar', 'whois_server',
                                       'referral_url'):
                    values = values[
                        -1]  # ignore junk   # 不要 'registrar', 'whois_server', 'referral_url' 这三个属性
                if len(values) == 1:  # 只有一个元素的时候 不使用列表表示
                    values = values[0]
                elif not values:  # 赋空值
                    values = None
                # 设置属性 及 属性值
    ...
    def _preprocess(self, attr, value):  # 处理value值
        value = value.strip()
        if value and isinstance(
                value, basestring) and not value.isdigit() and '_date' in attr:
            # try casting to date format
            value = cast_date(
                value, dayfirst=self.dayfirst, yearfirst=self.yearfirst)
        return value

    def __setitem__(self, name, value):  # 设置属性
        super(WhoisEntry, self).__setitem__(name, value)

    def __getattr__(self, name):  # 去某一特殊属性
...
# 每个WHOIS解析模板都由特别的属性及表达式组合而成,还包括
# 0. 属性及正则表达式集
# 1. 不存在情况下的检测
# 2. 额外的特殊处理
...
class WhoisOrg(WhoisEntry):
    ...
    regex = {
        ...
        'referral_url':
        'Referral URL: *(.+)',  # http url of whois_server: empty usually
        'updated_date': 'Updated Date: *(.+)',
        'creation_date': 'Creation Date: *(.+)',
...
class WhoisUs(WhoisEntry):
    ...
    regex = {
        ...
        'registrant_application_purpose':
        ...
        'admin_application_purpose':
        'Administrative Application Purpose: *(.+)',
...
class WhoisFi(WhoisEntry):
    ...
    regex = {
        ...
        'name_server_statuses':
        'nserver\.*: *([\S]+) \[\S+\]',  # list of name servers and statuses
        'dnssec': 'dnssec\.*: *([\S]+)',
    }
...
class WhoisEe(WhoisEntry):
    ...
    regex = {
        ...
        r'Registrar: *[\n\r]+\s*name: *([^\n\r]+)',
        'name_servers':
        r'nserver: *(.*)',  # list of name servers
    }
